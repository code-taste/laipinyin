...
#import "laipinyinApplicationDelegate.h"
#import "macos_keycode.h"
#import "imi_session_wrapper.h"

// forward declaration of 'Private' category
@interface laipinyinController(Private)
-(void)createSession;
-(void)destroySession;
@end 

// implementation of the public interface
@implementation laipinyinController
...
-(BOOL)handleEvent:(NSEvent*)event client:(id)sender
{
    ...
    laipinyinApplicationDelegate *pinyinDelegate = [laipinyinApplicationDelegate fromApp];
    ...
    switch ([event type]) {
        case NSFlagsChanged:
                ...
                !(_lastModifiers[0] & NSShiftKeyMask))
            {
                ...
                if (_englishMode) {
                    laipinyinApplicationDelegate *delegate = [laipinyinApplicationDelegate fromApp];
...
{
    if ([laipinyinApplicationDelegate fromApp].usingUSKbLayout)
...
{
    [[laipinyinApplicationDelegate fromApp].candiWin hideCandidates];
...
-(NSMenu*)menu
{
    return [laipinyinApplicationDelegate fromApp].menu;
}

-(void)showPrefPanel:(id)sender
{
    [[laipinyinApplicationDelegate fromApp] showPrefPanel:sender];
}

-(void)toggleChinesePuncts:(id)sender
{
    [[laipinyinApplicationDelegate fromApp] toggleChinesePuncts:sender];
    _session->setStatusAttrValue(CIMIWinHandler::STATUS_ID_FULLPUNC,
                                 [laipinyinApplicationDelegate fromApp].inputChinesePuncts);
}

-(void)toggleFullSymbols:(id)sender
{
    [[laipinyinApplicationDelegate fromApp] toggleFullSymbols:sender];
    _session->setStatusAttrValue(CIMIWinHandler::STATUS_ID_FULLSYMBOL,
                                 [laipinyinApplicationDelegate fromApp].inputFullSymbols);
}
...
-(void)showCandidates:(NSArray*)candidates
{
    ...
    [[laipinyinApplicationDelegate fromApp].candiWin showCandidates:candidates around:cursorRect];
}
...
-(void)updateStatus:(int)key withValue:(int)value
{
    ...
    case CIMIWinHandler::STATUS_ID_FULLPUNC:
        if (value != [laipinyinApplicationDelegate fromApp].inputChinesePuncts)
            [self toggleChinesePuncts:nil];
        break;
    case CIMIWinHandler::STATUS_ID_FULLSYMBOL:
        if (value != [laipinyinApplicationDelegate fromApp].inputFullSymbols)
...
@end // laipinyinController


// implementation of private interface
@implementation laipinyinController(Private)

-(void)createSession
{
    _session = new ClaipinyinSessionWrapper (self);
}
...
@end // laipinyinController(Private)
