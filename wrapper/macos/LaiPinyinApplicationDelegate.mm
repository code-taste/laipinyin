...
@implementation laipinyinApplicationDelegate
...
{
    return (laipinyinApplicationDelegate *)[NSApp delegate];
}
...
-(void)awakeFromNib
{
    ...
    if ([paths count]) 
    {
        ...
        user_data_dir.append ("/laipinyin");
...
//though we specified the showPrefPanel: in laipinyinApplicationDelegate as the
...
-(NSDictionary *)registrationDictionaryForGrowl
{
    ...
    notifications = [NSArray arrayWithObject: @"laipinyin"];
...
{
    [GrowlApplicationBridge notifyWithTitle: @"laipinyin"
                            description: msg
                            notificationName: @"laipinyin"
                            iconData: [NSData dataWithData:[[NSImage imageNamed:@"laipinyin"] TIFFRepresentation]]
                            priority: 0
                            isSticky: NO
                            clickContext: nil];
}

@end //laipinyinApplicationDelegate
...
void updateFactorySettings(NSUserDefaults* pref)
{
    ...
    factory.setPinyinScheme((ClaipinyinSessionFactory::EPyScheme)pinyin_scheme);
