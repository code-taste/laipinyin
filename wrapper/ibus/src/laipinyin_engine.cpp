...
#include "laipinyin_config_keys.h"
...
laipinyinEngine::laipinyinEngine(IBusEngine *engine)
    : m_engine(engine),
      m_status_prop(laipinyinProperty::create_status_prop(engine)),
      m_letter_prop(laipinyinProperty::create_letter_prop(engine)),
      m_punct_prop(laipinyinProperty::create_punct_prop(engine)),
      m_wh(NULL),
      m_pv(NULL),
      ...
      m_hard_forward(false)
{
    ...
    ClaipinyinSessionFactory::EPyScheme pinyin_scheme =
        m_config.get_py_scheme(ClaipinyinSessionFactory::QUANPIN);
    factory.setPinyinScheme(pinyin_scheme);
    if (pinyin_scheme == ClaipinyinSessionFactory::QUANPIN) {
...
laipinyinEngine::~laipinyinEngine()
{
    if (m_pv) {
        ClaipinyinSessionFactory& factory =
            ClaipinyinSessionFactory::getFactory();
...
gboolean
laipinyinEngine::process_key_event (guint key_val,
                                    guint key_code,
                                    guint modifiers)
{
...
void
laipinyinEngine::focus_out ()
{
    reset();
}

void
laipinyinEngine::reset ()
{
    m_pv->updateWindows(m_pv->clearIC());
}

void
laipinyinEngine::enable ()
{
...
void
laipinyinEngine::disable ()
{
}

void
laipinyinEngine::page_up ()
{
    m_pv->onCandidatePageRequest(-1, true /* relative */);
}

void
laipinyinEngine::page_down ()
{
    m_pv->onCandidatePageRequest(1, true /* relative */);
}

void
laipinyinEngine::property_activate (const std::string& property, unsigned /*state*/)
{
...
void
laipinyinEngine::candidate_clicked (guint index)
{
    m_pv->onCandidateSelectRequest(index);
}

void
laipinyinEngine::cursor_up ()
{
...
bool
laipinyinEngine::onConfigChanged(const COptionEvent& event)
{
...
void
laipinyinEngine::commit_string (const std::wstring& str)
{
    IBusText *text;
...
void
laipinyinEngine::update_candidates(const ICandidateList& cl)
{
...
void
laipinyinEngine::update_lookup_table()
{
    ibus_engine_update_lookup_table(m_engine, m_lookup_table, TRUE);
}

bool
laipinyinEngine::is_valid() const
{
    return m_pv != NULL;
}
...
void
laipinyinEngine::update_preedit_string(const IPreeditString& preedit)
{
    if (preedit.size() > 0) {
...
void
laipinyinEngine::update_status_property(bool cn)
{
    m_status_prop.update(cn);
}

void
laipinyinEngine::update_punct_property(bool full)
{
    m_punct_prop.update(full);
}

void
laipinyinEngine::update_letter_property(bool full)
{
    m_letter_prop.update(full);
}

void
laipinyinEngine::update_history_power()
{
...
void
laipinyinEngine::update_charset_level()
{
...
void
laipinyinEngine::update_cand_window_size()
{
...
void
laipinyinEngine::update_mode_key()
{
...
void
laipinyinEngine::update_punct_key()
{
...
void
laipinyinEngine::update_page_key_minus()
{
...
void
laipinyinEngine::update_page_key_comma()
{
...
void
laipinyinEngine::update_page_key_bracket()
{
...
void
laipinyinEngine::update_page_key(const char* conf_key, bool default_val,
...
void
laipinyinEngine::update_cancel_with_backspace()
{
...
void
laipinyinEngine::update_smart_punc()
{
    m_pv->setSmartPunct(m_config.get(CONFIG_KEYBOARD_SMARK_PUNCT, true));
}

void
laipinyinEngine::update_max_best()
{
    if (m_pv->getIC() == NULL) {
        return;
    }
...
void
laipinyinEngine::update_max_tail_candidate()
{
    if (m_pv->getIC() == NULL) {
        return;
    }
...
void
laipinyinEngine::update_punct_mappings()
{
...
void
laipinyinEngine::update_correction_pinyins()
{
...
void
laipinyinEngine::update_fuzzy_segs()
{
...
void
laipinyinEngine::update_shuangpin_type()
{
...
void
laipinyinEngine::update_candi_delete_key()
{
