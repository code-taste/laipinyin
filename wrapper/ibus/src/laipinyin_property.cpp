...
laipinyinProperty
laipinyinProperty::create_status_prop(ibus::Engine engine, bool state)
{
    laipinyinProperty prop(engine, PROP_STATUS);
    prop.m_info[0].label = ibus_text_new_from_static_string("EN");
    prop.m_info[0].icon  = IBUS_laipinyin_ICON_DIR"/eng.svg";
    ...
    prop.m_info[1].icon  = IBUS_laipinyin_ICON_DIR"/han.svg";
...
laipinyinProperty
laipinyinProperty::create_letter_prop(ibus::Engine engine, bool state)
{
    laipinyinProperty prop(engine, PROP_LETTER);
    prop.m_info[0].label = ibus_text_new_from_static_string("Aa");
    prop.m_info[0].icon  = IBUS_laipinyin_ICON_DIR"/halfwidth.svg";
    ...
    prop.m_info[1].icon  = IBUS_laipinyin_ICON_DIR"/fullwidth.svg";
...
laipinyinProperty
laipinyinProperty::create_punct_prop(ibus::Engine engine, bool state)
{
    laipinyinProperty prop(engine, PROP_PUNCT);
    prop.m_info[0].label = ibus_text_new_from_static_string(",.");
    prop.m_info[0].icon  = IBUS_laipinyin_ICON_DIR"/enpunc.svg";
    ...
    prop.m_info[1].icon  = IBUS_laipinyin_ICON_DIR"/cnpunc.svg";
...
laipinyinProperty::laipinyinProperty(ibus::Engine engine, const std::string& name)
    : ibus::Property(
...
laipinyinProperty::~laipinyinProperty()
{
}

bool
laipinyinProperty::toggle(const std::string& name)
{
    if (name == m_name) {
...
void
laipinyinProperty::update(bool state)
{
    if (state == m_state)
        return;
    init(state);
...
void
laipinyinProperty::init(bool state)
{
    m_state = state;
      ...
      m_name("setup")
{
    ...
    m_info.icon    = IBUS_laipinyin_ICON_DIR"/setup.svg";
    init();
}
...
SetupLauncher::launch(const std::string& name)
{
    ...
    path = g_build_filename(libexecdir, "ibus-setup-laipinyin", NULL);
    argv[0] = path;
    argv[1] = NULL;
    gboolean success;
