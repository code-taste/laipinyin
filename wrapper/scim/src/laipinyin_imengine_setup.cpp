...
#include "laipinyin_private.h"
#include "laipinyin_imengine_config_keys.h"

using namespace scim;

#if !GTK_CHECK_VERSION(2, 12, 0)
    #define laipinyin_USE_GTK_TOOLTIPS
#endif

#if GTK_CHECK_VERSION(2, 24, 0)
    #define laipinyin_USE_GTK_COMBO_BOX_TEXT
#endif

#if GTK_CHECK_VERSION(3, 0, 0)
    #define laipinyin_USE_GTK_BOX
#endif

#define scim_module_init laipinyin_imengine_setup_LTX_scim_module_init
#define scim_module_exit laipinyin_imengine_setup_LTX_scim_module_exit

#define scim_setup_module_create_ui       laipinyin_imengine_setup_LTX_scim_setup_module_create_ui
#define scim_setup_module_get_category    laipinyin_imengine_setup_LTX_scim_setup_module_get_category
#define scim_setup_module_get_name        laipinyin_imengine_setup_LTX_scim_setup_module_get_name
#define scim_setup_module_get_description laipinyin_imengine_setup_LTX_scim_setup_module_get_description
#define scim_setup_module_load_config     laipinyin_imengine_setup_LTX_scim_setup_module_load_config
#define scim_setup_module_save_config     laipinyin_imengine_setup_LTX_scim_setup_module_save_config
#define scim_setup_module_query_changed   laipinyin_imengine_setup_LTX_scim_setup_module_query_changed
...
extern "C" {
    ...
    {
        bindtextdomain (GETTEXT_PACKAGE, SCIM_laipinyin_LOCALEDIR);
...
static GtkWidget *
#ifdef laipinyin_USE_GTK_TOOLTIPS
...
#ifdef laipinyin_USE_GTK_COMBO_BOX_TEXT
...
load_config (const ConfigPointer &config)
{
    ...
    int viewtype = config->read (String (SCIM_CONFIG_IMENGINE_laipinyin_USER_VIEW_TYPE), 0);
    gtk_combo_box_set_active (GTK_COMBO_BOX(input_style_combo), viewtype);

    int charset  = config->read (String (SCIM_CONFIG_IMENGINE_laipinyin_USER_CHARHSET), 1);
    gtk_combo_box_set_active (GTK_COMBO_BOX(charset_combo), charset);

    bool stat;
    stat = config->read (String (SCIM_CONFIG_IMENGINE_laipinyin_USER_PAGE_MINUS), true);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(minus_pageup_button), stat);

    stat = config->read (String (SCIM_CONFIG_IMENGINE_laipinyin_USER_PAGE_BRACKET), true);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(bracket_pageup_button), stat);

    stat = config->read (String (SCIM_CONFIG_IMENGINE_laipinyin_USER_PAGE_COMMA), false);
    gtk_toggle_button_set_active(GTK_TOGGLE_BUTTON(comma_pageup_button), stat);

    gint value;
    value = config->read (String (SCIM_CONFIG_IMENGINE_laipinyin_USER_MEMORY_POWER), 5);
...
save_config (const ConfigPointer &config)
{
    ...
    config->write (String (SCIM_CONFIG_IMENGINE_laipinyin_USER_VIEW_TYPE), no);

    no = gtk_combo_box_get_active(GTK_COMBO_BOX(charset_combo));
    config->write (String (SCIM_CONFIG_IMENGINE_laipinyin_USER_CHARHSET), no);

    gboolean stat;
    stat = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(minus_pageup_button));
    config->write (String (SCIM_CONFIG_IMENGINE_laipinyin_USER_PAGE_MINUS), (bool)stat);

    stat = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(bracket_pageup_button));
    config->write (String (SCIM_CONFIG_IMENGINE_laipinyin_USER_PAGE_BRACKET), (bool)stat);

    stat = gtk_toggle_button_get_active(GTK_TOGGLE_BUTTON(comma_pageup_button));
    config->write (String (SCIM_CONFIG_IMENGINE_laipinyin_USER_PAGE_COMMA), (bool)stat);

    gint value;
    value = gtk_spin_button_get_value_as_int(GTK_SPIN_BUTTON(memory_power_button));
    config->write (String (SCIM_CONFIG_IMENGINE_laipinyin_USER_MEMORY_POWER), value);
    __have_changed = false;
}
